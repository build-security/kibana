...
interface BaseRulesSearchBarProps {
  dataView: DataView;
  onError(v: string): void;
  onSuccess(v: CSPRule[]): void;
  onLoading(): void;
}

type RulesSearchBarProps = BaseRulesSearchBarProps & FetchState<CSPRule[]>;
...
export const RulesSearchBar = ({
  ...
  const runSearch = useCallback(async () => {
    ...
    query.filterManager.setFilters([...filters].filter(isNonNullable));

    try {
      const rulesSearchSource = await search.searchSource.create({
      ...
      const rulesResponse: IKibanaSearchResponse<SearchResponse<CSPRule>> = await rulesSearchSource
        .fetch$()
        .toPromise();
      onSuccess(rulesResponse.rawResponse.hits.hits.map((v) => v._source).filter(isNonNullable));
    } catch (e) {
      onError(e);
    }
  }, [
